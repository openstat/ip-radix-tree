...
public class BenchmarkTests {
    ...
    public void benchmarkFromFileTest() throws IOException {
        ...
        IPv4RadixTree[] tr = new IPv4RadixTree[M];
        for( int i = 0; i < M; i++ ) {
            tr[i] = IPv4RadixTree.loadFromLocalFile( "src/test/resources/ip-prefix-base.txt" );
        }
        gc();
    ...
    @Test
    public void benchmarkRandomTest() {
        Random rnd = new Random( 42 );

        gc();
        long m1 = Runtime.getRuntime().freeMemory();

        IPv4RadixTree tr = new IPv4RadixTree( 500000 );

        for( int i = 0; i < N; i++ ) {
            int cidr = rnd.nextInt( 33 );
            long netmask = ((1L << (32 - cidr)) - 1L) ^ 0xffffffffL;
            tr.put( rnd.nextLong(), netmask );
        }
